...
site_list = ["CA", "SD", "SF", "HA", "YA", "LA", "WU", "PI", "PA", "OR", "NN", "IR", "NL", "NN", "NC", "TE", "MT"]
...
for id in id_list:
	...
	if 'chrdemo_age_yrs_chr' in sub_data_baseline and pd.notna(sub_data_baseline.at[0, "chrdemo_age_yrs_chr"]):
		...
		sub_data_baseline.at[0, 'interview_age'] = sub_data_baseline.at[0, "chrdemo_age_yrs_chr"]
	...
	if 'chrdemo_age_yrs_hc' in sub_data_baseline and pd.notna(sub_data_baseline.at[0, "chrdemo_age_yrs_hc"]):
		...
		sub_data_baseline.at[0, 'interview_age'] = sub_data_baseline.at[0, "chrdemo_age_yrs_hc"]
...
# month1 visit
final_month1_csv = pd.concat(id_month1_tracker, ignore_index=True)
numbers = list(range(1,(len(final_month1_csv.index) +1))) # changing day numbers to sequence
final_month1_csv['day'] = numbers
final_month1_csv = final_month1_csv.sort_values(['days_since_consent', 'day'])
final_month1_csv['day'] = numbers
...
#final_month1_csv.to_csv(output1 + "combined-PRONET-form_month1-day1to1.csv", sep=',', index = False, header=True)

print("Final combined Pronet csvs")
print("Screening")
print(final_csv.T.iloc[:15,:])
print("Baseline")
print(final_baseline_csv.T.iloc[:15,:])
print("Month1")
print(final_month1_csv.T.iloc[:15,:])
