...
baseline_prescient_complete = baseline_prescient[['subjectid', 'chrdemo_sexassigned']]
...
## screening complete
screening_prescient_complete = pd.merge(screening_prescient_1, baseline_prescient_complete, on=['subjectid'])
screening_prescient_complete['Network'] = "PRESCIENT"
print(screening_prescient_complete)
#screening_prescient_complete.to_csv(output + "Summary_PRESCIENT_forms_" + today + ".csv", sep=',', index = False, header=True)
...
baseline_pronet_complete = baseline_pronet[['subjectid','chrdemo_sexassigned']]
...
## Screening 
screening_pronet_complete = pd.merge(screening_pronet_1, baseline_pronet_complete, on=['subjectid'])
screening_pronet_complete['Network'] = "ProNET"
print(screening_pronet_complete)
#screening_pronet_complete.to_csv(output + "Summary_ProNET_forms_" + today + ".csv", sep=',', index = False, header=True)
...
ampscz = pd.concat([screening_pronet_complete, screening_prescient_complete],axis=0, ignore_index=True)

ampscz = ampscz.rename(columns={'chrguid_guid': 'GUID', 'chrcrit_part': 'Group',  'visit_status_string': 'Current_visit', 'chrdemo_sexassigned': 'Sex_at_birth'})
...
# saving summary csv
ampscz.to_csv(output + "Summary_AMP-SCZ_forms_" + today + ".csv", sep=',', index = False, header=True)
