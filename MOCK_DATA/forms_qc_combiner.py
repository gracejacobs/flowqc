...
input_path = "/data/predict/data_from_nda_dev/formqc/"
print("Input path: " + input_path + "\n")

output_path = "/data/predict/data_from_nda_dev/formqc/"
print("Output path: " + output_path)

reference = pd.read_csv(input_path + 'formqc-YA00037-forms_qc-day1to2.csv')
#print(reference)
...
# removing first row because it's a duplicate
all_screening = all_screening.iloc[1: , :]
...
screening.to_csv(output_path+"combined_screen-forms_qc-day1to9999.csv", sep=',', index = False, header=True)

# Just baseline data
numbers = list(range(1,(len(baseline.index) +1)))
baseline['day'] = numbers
baseline.to_csv(output_path+"combined_baseline-forms_qc-day1to9999.csv", sep=',', index = False, header=True)

# All data for all subjects
numbers = list(range(1,(len(all_screening.index) +1)))
all_screening['day'] = numbers
all_screening.to_csv(output_path+"combined_all-forms_qc-day1to9999.csv", sep=',', index = False, header=True)
